            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="detailQTitle" readonly>
                    </div>
                    <div class="form-group">
...
<script>
    ...
    async function loadPageData(id) {
        try {
            ...
            const answerPromise = axios.get(`${READ_API_BASE}/response/${id}`).catch(e => null); // 답변 없으면 null

            // --- 모든 요청이 완료될 때까지 대기 ---
            const [quotationRes, answerRes] = await Promise.all([quotationPromise, answerPromise]);

            // --- 1. 상세 정보 렌더링 (DTO 속성 반영) ---
            const quotation = quotationRes.data; // QuotationDetailDTO
            ...
            const answer = answerRes ? answerRes.data : null; // QuotationResponseDTO
            if (answer) {
                currentAnswerId = answer.qr_index; // DTO 속성: qr_index
    ...
    function renderDynamicUI(quotation, answer) {
        ...
        if (loginUserType === 'ADMIN' || loginUserId === String(quotation.user_index)) {
        ...
        if (loginUserType === 'ADMIN') {
            ...
            if (answer) {
                ...
                answerFormContent.value = answer.qr_content; // DTO 속성: qr_content
                deleteAnswerBtn.style.display = "inline-block"; // 삭제 버튼 표시
            } else {
                // 답변 등록
    ...
    async function loadComments(quotationId, page) {
        ...
        try {
            // [API 경로 수정]: 컨트롤러 경로(/comment/{id}) 반영
            const params = new URLSearchParams({ page, amount: 5 }); // 댓글 5개씩
    ...
    function bindModalEvents() {
        ...
        document.getElementById("updateQuotationBtn").addEventListener("click", function() {
            ...
            axios.put(`${WRITE_API_BASE}/request/${currentQuotationId}`, data, { headers: { 'Content-Type': 'application/json' } })
                .then(response => {
                    alert("견적이 수정되었습니다.");
        ...
        const ADMIN_API_BASE = API.ADMIN; // 답변 관련은 무조건 Admin API
        ...
        document.getElementById("saveAnswerBtn").addEventListener("click", function() {
            ...
            // currentAnswerId가 있으면 수정(PUT), 없으면 등록(POST)
            // [API 경로 수정]: /response
        ...
        document.getElementById("saveCommentBtn").addEventListener("click", function() {
            ...
            axios.post(`${WRITE_API_BASE}/comment`, data, {
                ...
                .then(response => {
                    ...
                    loadComments(currentQuotationId, 1); // 댓글 목록만 새로고침
                })
                .catch(error => alert("댓글 등록 실패: " + (error.response?.data?.message || "서버 오류")));
        });
