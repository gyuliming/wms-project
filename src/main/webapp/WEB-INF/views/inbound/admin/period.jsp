  ...
  <div class="page-inner">
    <div class="page-header">
      ...
      <ul class="breadcrumbs mb-3">
        ...
        <li class="separator"><i class="icon-arrow-right"></i></li>
        <li class="nav-item">기간별 현황</li>
      </ul>
    </div>

    <div class="card mb-4">
      <div class="card-body">
        <form id="periodSearchForm" onsubmit="event.preventDefault(); loadPeriodStatus();">
    ...
    <div class="card">
      <div class="card-header">
        <h4 class="card-title">기간별 현황 결과</h4>
...
<script>
  ...
  function loadPeriodStatus() {
    ...
    const url = '<c:url value="/inbound/admin/status/period"/>?startDate=' + startDate + '&endDate=' + endDate;
    // 로딩 표시
    ...
    fetch(url, {
      ...
      headers: {
        'Content-Type': 'application/json'
            ...
            .then(data => {
              displayPeriodResults(data, startDate, endDate);
            })
            .catch(error => {
  ...
  // 결과 표시
  function displayPeriodResults(results, startDate, endDate) {
    const tbody = document.getElementById('periodResults');
    const summary = document.getElementById('resultSummary');

    if (!results || results.length === 0) {
      tbody.innerHTML = '<tr><td colspan="8" class="text-center py-4 text-muted">해당 기간에 입고 요청 내역이 없습니다.</td></tr>';
      summary.textContent = startDate + ' ~ ' + endDate + ' - 결과 없음';
      return;
    }
    ...
    results.forEach(item => {
      ...
      if (item.approvalStatus && statusCount.hasOwnProperty(item.approvalStatus)) {
        statusCount[item.approvalStatus]++;
      }
      ...
      const plannedDate = item.plannedReceiveDate ?
              formatDate(item.plannedReceiveDate) : '-';
    ...
    // 요약 정보
    summary.innerHTML = startDate + ' ~ ' + endDate + ' - 총 <strong>' + totalRequests + '건</strong> | ' +
            '요청수량: <strong>' + totalQuantity + '개</strong> | ' +
            '입고수량: <strong>' + totalReceived + '개</strong> | ' +
            '<span class="badge bg-warning ms-2">대기 ' + statusCount.PENDING + '</span> ' +
            '<span class="badge bg-success">승인 ' + statusCount.APPROVED + '</span> ' +
            '<span class="badge bg-danger">거부 ' + statusCount.REJECTED + '</span> ' +
            '<span class="badge bg-secondary">취소 ' + statusCount.CANCELED + '</span>';
  }
